...
can_frame_lib MMeetFrameGenerator::init_frame(uint8_t module_address) {
    can_frame_lib frame{};
...
can_frame_lib MMeetFrameGenerator::create_command_frame(uint8_t module_address, uint16_t command) {
    can_frame_lib frame = init_frame(module_address);
...
can_frame_lib MMeetFrameGenerator::create_control_frame(uint8_t module_address, uint16_t command, uint16_t value) {
    can_frame_lib frame = create_command_frame(module_address, command);
...
can_frame_lib MMeetFrameGenerator::generateTempRequest(uint8_t module_address) {
    return create_command_frame(module_address, MMeetConstants::TEMP_CMD);
}

can_frame_lib MMeetFrameGenerator::generateCurrentCapabilityRequest(uint8_t module_address) {
    return create_command_frame(module_address, MMeetConstants::CURRENT_CAP_CMD);
}

can_frame_lib MMeetFrameGenerator::generateFlagsRequest(uint8_t module_address) {
    return create_command_frame(module_address, MMeetConstants::FLAGS_CMD);
}

can_frame_lib MMeetFrameGenerator::generateVoltageRequest(uint8_t module_address) {
    return create_command_frame(module_address, MMeetConstants::VOLTAGE_CMD);
}

can_frame_lib MMeetFrameGenerator::generateCurrentRequest(uint8_t module_address) {
    return create_command_frame(module_address, MMeetConstants::CURRENT_CMD);
}

can_frame_lib MMeetFrameGenerator::generateLowModeSet(uint8_t module_address) {
    return create_control_frame(module_address, MMeetConstants::MODE_SET_CMD, MMeetConstants::LOW_MODE);
}

can_frame_lib MMeetFrameGenerator::generateHighModeSet(uint8_t module_address) {
    return create_control_frame(module_address, MMeetConstants::MODE_SET_CMD, MMeetConstants::HIGH_MODE);
}

std::optional<can_frame_lib> MMeetFrameGenerator::generateAutoModeSet(uint8_t module_address) {
    return create_control_frame(module_address, MMeetConstants::MODE_SET_CMD, MMeetConstants::AUTO_MODE);
}

can_frame_lib MMeetFrameGenerator::generateVoltageSet(uint8_t module_address, float voltage) {
    uint32_t math_voltage = static_cast<uint32_t>(voltage * 100);
    can_frame_lib frame = create_command_frame(module_address, MMeetConstants::VOLTAGE_SET_CMD);
    pack_uint32(frame.data + 4, math_voltage);
    return frame;
}

can_frame_lib MMeetFrameGenerator::generateCurrentSet(uint8_t module_address, float current) {
    uint32_t math_current = static_cast<uint32_t>(current * 1000);
    can_frame_lib frame = create_command_frame(module_address, MMeetConstants::CURRENT_SET_CMD);
    pack_uint32(frame.data + 4, math_current);
    return frame;
}

can_frame_lib MMeetFrameGenerator::generateEnable(uint8_t module_address) {
    can_frame_lib frame = create_command_frame(module_address, MMeetConstants::POWER_CTRL_CMD);
    frame.data[7] = MMeetConstants::ON;
    return frame;
}

can_frame_lib MMeetFrameGenerator::generateDisable(uint8_t module_address) {
    can_frame_lib frame = create_command_frame(module_address, MMeetConstants::POWER_CTRL_CMD);
